 ...
 */
public class GSvgIcon implements Icon {
	private static WeakStore<GSvgIcon> inUseIcons = new WeakStore<>();
	...
	public static void refreshAll(GThemeValueMap currentValues) {
		for (GSvgIcon gIcon : inUseIcons.getValues()) {
	...
	private URL getUrl(Icon icon) {
		...
		else if (icon instanceof GSvgIcon) {
			return getUrl(((GSvgIcon) icon).delegate);
		}
